...
public final class XmlUtils {
	 ...
	 */
	public static List<String> readTextFromXML(String file,
		...
		try {
			...
			NodeList resultNodes = (NodeList) expression.evaluate(document,
					XPathConstants.NODESET);
			for (int i = 0; i < resultNodes.getLength(); i++) {
				reuslts.add(resultNodes.item(i).getTextContent());
			}
		} catch (Exception ex) {
			logger.warn("Error parsing file: " + file + " for xpath: "
					+ xpathExpression, ex);
		}
		return reuslts;
	}
	 ...
	 *            xpath
	 * @param attribute
	 *            attribute name to get value for
	 * @return value of the requested attribute in the xpath defined
	 */
	public static List<String> readAttributeFromXML(String file,
			String xpathExpression, String attribute) {
		List<String> reuslts = new ArrayList<String>();
		try {
			DocumentBuilderFactory factory = DocumentBuilderFactory
					.newInstance();
			...
			NodeList resultNodes = (NodeList) expression.evaluate(document,
					XPathConstants.NODESET);
			for (int i = 0; i < resultNodes.getLength(); i++) {
				Node attributeNode = resultNodes.item(i).getAttributes()
			...
			logger.warn("Error parsing file: " + file + " for xpath: "
					+ xpathExpression + " attribute:" + attribute, ex);
		}
		return reuslts;
	}
	...
	public static String readValueFromXML(String file, String elementName) {
		...
		try {
			...
			logger.warn("Error parsing file: " + file + " for element: "
					+ elementName, ex);
		}
		return result;
	}
