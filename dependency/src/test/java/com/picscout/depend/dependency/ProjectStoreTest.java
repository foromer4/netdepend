...
public class ProjectStoreTest {
	...
	public void testGetProjectByAssmblyEmpty() {
		assertNull("no project should return",
				store.getProjectByAssemblyName(assemblyName));
	}

	@Test
	public void testGetProjectByGuidEmpty() {
		assertNull("no project should return",
				store.getProjectByGuid(projectGuid));
	}
	...
	public void testGetProjectByAssmblyWrongName() {
		...
		assertNull("no project should return",
				store.getProjectByAssemblyName(projectGuid));
	}
	...
	public void testGetProjectByGuidWrongName() {
		store.addProject(project);
		assertNull("no project should return",
				store.getProjectByGuid(assemblyName));
	}

	@Test
	public void testGetProjectByAssmblyCorrectName() {
		store.addProject(project);
		assertEquals("A project should return",
				store.getProjectByAssemblyName(assemblyName), project);
	}
	...
	public void testGetProjectByGuidCorrectName() {
		store.addProject(project);
		assertEquals("A project should return",
				store.getProjectByGuid(projectGuid), project);
	}

	@Test
	public void testGetProjectCountWhenEmpty() {
		assertEquals("no projects are mapped", store.getAllMappedProjects()
				.size(), 0);
	}
